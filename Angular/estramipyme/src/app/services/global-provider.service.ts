...
export class GlobalProviderService {
  ...
  constructor() {
    this.dataProc.getItems('http://localhost:3000/questions').subscribe({
      next: questions => {
        ...
        this.circuloQue = questions.filter(e => e.section === 'circulo' && e.subsection === "que").map(q => String(q.id));
        this.circuloComo = questions.filter(e => e.section === 'circulo' && e.subsection === "como").map(q => String(q.id));
        this.circuloPorQue = questions.filter(e => e.section === 'circulo' && e.subsection === "por que").map(q => String(q.id));
  ...
  getCircle() {
    ...
    return {
      ...
      why: [porQueAcc / (total * 4) * 100, 100 - (porQueAcc / (total * 4) * 100)],
    }
  }
